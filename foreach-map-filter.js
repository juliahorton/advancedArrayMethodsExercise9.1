...
function vowelCount(str){
   ...
   newArr.forEach(function(char) {
    if (vowels.includes(char) && newObj[char] === undefined) {
        newObj[char] = 1;
...
function removeVowels(str) {
    const newArr = Array.from(str.toLowerCase());
    let vowels = "aeiou";
    const filteredArr = newArr.filter(function(char) {
...
function doubleOddNumbers(arr) {
    const oddsOnly = arr.filter(function(num) {
        return !(num % 2 === 0)
    });
    return oddsOnly.map(function(num) {
        return num * 2;
    });
}
